...
**Token estimate before an AI search** (`POST /api/match/estimate`, `estimate_tokens`): when the toggle is on, clicking "Find Matches" first calls this endpoint and shows a preview panel; the real search runs only after "Run AI search" (editing the description, toggling, or Cancel discards the preview). Input tokens come from the free `messages.count_tokens` endpoint using the exact same request as the real call (`_request_params` is shared, so keep both paths going through it) and are exact. Output tokens can't be known in advance (adaptive thinking + variable reasoning length), so they're a range from the `OUTPUT_*` heuristic constants scaled by candidate count — recalibrate them against real `usage` if the prompt, schema or model changes. Cost uses the `*_USD_PER_MTOK` constants (Sonnet 5 list prices). If counting fails, the endpoint returns 503 and the UI offers "Run anyway". After an AI search, `/api/match` returns actual `usage` (`input_tokens`, `output_tokens`, `cost_usd`), shown next to the "AI-ranked" hint.

**Response shape** (both engines): `job_description_skills`, `results[]` (`id, candidate_name, original_filename, matched_skills, missing_skills, score, match_percentage`, plus `reasoning` for the LLM engine; for the LLM `score == match_percentage == fit_score`), `excluded_count`, `note`, `engine` (`"llm"`, `"keyword"`, or `null` when no candidates are reviewed), and `usage` (LLM engine only).

**Frontend**: `JobMatch.jsx` has a "Use Claude AI for matching" checkbox (default **off**, so searches cost nothing unless the user opts in) sent as `use_ai` via `matchCandidates(jobDescription, useAi)` in `api.js`. It shows Claude's required skills as chips, an "AI-ranked" hint when `engine === "llm"`, the per-candidate `reasoning`, and "Analyzing…" while the call is in flight. With the toggle on, submit first calls `estimateMatch` (`POST /api/match/estimate`) and renders the estimate panel (`estimate` state: `{data}` or `{error}`); `runSearch` does the actual match. With the toggle off, submit calls `runSearch` directly.
...
- `HowItWorks.jsx` renders a section's `<h2>` plus a "How it works" toggle that reveals an explanation panel (`children`, styled by `.help-panel` / `.help-callout` in `index.css`). `UploadForm` and `JobMatch` use it in place of their headings. The panel text describes data handling and AI usage for end users — keep it in sync when changing extraction, matching, the AI toggle default, or what is sent to Claude.
