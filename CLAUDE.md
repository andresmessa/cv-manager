...
**Frontend**: `JobMatch.jsx` has a "Use Claude AI for matching" checkbox (default on) sent as `use_ai` via `matchCandidates(jobDescription, useAi)` in `api.js`. It shows Claude's required skills as chips, an "AI-ranked" hint when `engine === "llm"`, the per-candidate `reasoning`, and "Analyzing…" while the call is in flight. With the toggle on, submit first calls `estimateMatch` (`POST /api/match/estimate`) and renders the estimate panel (`estimate` state: `{data}` or `{error}`); `runSearch` does the actual match. With the toggle off, submit calls `runSearch` directly.
